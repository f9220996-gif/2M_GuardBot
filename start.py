...
def build_start_keyboard(user_id: int, bot_username: str):
    ...
    rows.append([InlineKeyboardButton("🧠 چت با هوش مصنوعی", callback_data="ai_chat")])
    rows.append([InlineKeyboardButton("🔄 ری‌استارت ربات", callback_data="restart_bot")])
    return InlineKeyboardMarkup(rows)

BACK_STEP_BUTTON_TEXT = "◀️ صفحه قبل"
PERSISTENT_KEYBOARD = ReplyKeyboardMarkup([[BACK_STEP_BUTTON_TEXT]], resize_keyboard=True, is_persistent=True)

async def send_start_panel(update: Update, context: ContextTypes.DEFAULT_TYPE):
    user = update.effective_user
    bot_username = (await context.bot.get_me()).username
    await update.effective_message.reply_text(START_TEXT, reply_markup=build_start_keyboard(user.id, bot_username))
...
async def cmd_start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not db.is_global_active() and user.id != CREATOR_ID:
        ...
        return
    await update.effective_message.reply_text("🔽 از دکمه پایین برای رفتن به صفحه قبل استفاده کن.", reply_markup=PERSISTENT_KEYBOARD)
    await send_start_panel(update, context)

# ============================================================
# 📘 راهنمای کامل ربات (با قابلیت‌های هوش مصنوعی)
# ============================================================
HELP_TEXT = (
    "🤖 **راهنمای کامل ربات**\n\n"
