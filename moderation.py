...
from utils.permissions import can_use_moderation_commands, can_target_user, parse_duration_seconds
from utils.persian_date import build_restriction_message, build_duration_text
...
async def cmd_azad_kon(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    username = f"@{target.username}" if target.username else target.full_name
    await _reply(update, f"🔊 سکوت {username} برداشته شد.")
...
async def cmd_pak(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        await _reply(update, f"❌ نتونستم پیام رو پاک کنم.\n{e}")
