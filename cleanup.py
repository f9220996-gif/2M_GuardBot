...
با قابلیت تنظیم دلخواه (روز/ساعت/دقیقه) و کم/زیاد کردن با دکمه‌های شیشه‌ای
"""
...
async def ask_interval(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """نمایش دکمه‌های تنظیم بازه زمانی (روز/ساعت/دقیقه)"""
    query = update.callback_query
...
async def adjust_interval(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if action == "plus":
        ...
        interval_seconds = max(60, interval_seconds - step)
...
async def set_cleanup_count(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    text = f"📊 **تعداد پیام‌ها**\n\nمقدار فعلی: {count}\n\nبا دکمه‌های زیر تنظیم کنید:"
    
    kb = InlineKeyboardMarkup([
        [
            InlineKeyboardButton("➖", callback_data=f"cln_count_adjust:{chat_id}:minus"),
            InlineKeyboardButton(f"{count}", callback_data="cln_dummy"),
            InlineKeyboardButton("➕", callback_data=f"cln_count_adjust:{chat_id}:plus"),
        ],
        [
            InlineKeyboardButton("⬅️ بازگشت", callback_data=f"cln_panel:{chat_id}"),
        ],
...
async def adjust_count(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await query.answer("✔ ذخیره شد")
    await set_cleanup_count(update, context)
