...
def _group_panel_keyboard(chat_id, group):
    ...
    return InlineKeyboardMarkup([
        ...
        [InlineKeyboardButton("🧩 روشن/خاموش قابلیت‌ها", callback_data=f"grp_features:{chat_id}")],
...
async def set_mute_duration_from_panel(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    from telegram import ChatPermissions
    until_dt = datetime.fromtimestamp(time.time() + minutes * 60)
...
async def show_features_panel(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    for key, label in db.TOGGLEABLE_FEATURES.items():
        ...
        line.append(InlineKeyboardButton(f"{icon} {label}", callback_data=f"feat_toggle:{chat_id}:{key}"))
        if len(line) == 2:
            rows_kb.append(line)
            line = []
    if line:
...
async def toggle_feature(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    warning = None
    if feature_key in ("photos", "videos", "documents"):
        # این‌ها علاوه بر ذخیره داخلی، واقعاً دسترسی گروه رو هم از تلگرام تغییر می‌دن
        from telegram import ChatPermissions
        try:
            current_chat = await context.bot.get_chat(chat_id)
            p = current_chat.permissions
            await context.bot.set_chat_permissions(
                chat_id,
                ChatPermissions(
                    can_send_messages=p.can_send_messages if p else True,
                    can_send_audios=p.can_send_audios if p else True,
                    can_send_documents=(new_state if feature_key == "documents" else (p.can_send_documents if p else True)),
                    can_send_photos=(new_state if feature_key == "photos" else (p.can_send_photos if p else True)),
                    can_send_videos=(new_state if feature_key == "videos" else (p.can_send_videos if p else True)),
                    can_send_video_notes=p.can_send_video_notes if p else True,
                    can_send_voice_notes=p.can_send_voice_notes if p else True,
                    can_send_polls=p.can_send_polls if p else True,
                    can_send_other_messages=p.can_send_other_messages if p else True,
                    can_add_web_page_previews=p.can_add_web_page_previews if p else True,
                )
            )
        except Exception:
            warning = "⚠️ ذخیره شد ولی نتونستم دسترسی گروه رو از تلگرام عوض کنم (چک کن ربات ادمینه)."

    await query.answer(warning or ("روشن شد ✅" if new_state else "خاموش شد ❌"), show_alert=bool(warning))
