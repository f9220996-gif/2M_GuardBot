...
processed = set()
sent_cache = {}  # کش برای تشخیص پیام‌های تکراری ربات
...
print("✅ ربات M2_GuardBot با پاک کردن پیام تکراری روشن شد!")
...
while True:
    try:
        ...
        for update in r.get("result", []):
            ...
            # اگه فرستنده خود ربات باشه و پیام تکراری باشه
            ...
            if chat_type == "private":
                if text == "/start":
                    if user_id == OWNER_ID:
                        send_msg(chat_id, HELP_TEXT, reply_to=msg_id, delete_after=0)
                    continue
            else:
                if text == "/start":
                    continue
