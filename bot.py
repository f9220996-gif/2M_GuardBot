...
processed = set()
last_msg_time = {}  # برای جلوگیری از ارسال سریع

def send_msg(chat_id, text, reply_to=None, delete_after=60):
    """ارسال پیام با قفل جلوگیری از ارسال دوباره"""
    # جلوگیری از ارسال پیام‌های تکراری در کمتر از 1 ثانیه
    key = f"{chat_id}_{text[:20]}"
    if key in last_msg_time and time.time() - last_msg_time[key] < 1.5:
        return None
    last_msg_time[key] = time.time()
...
def delayed_delete(chat_id, msg_id, delay):
    time.sleep(delay)
    try:
        requests.post(f"https://api.telegram.org/bot{TOKEN}/deleteMessage", data={"chat_id": chat_id, "message_id": msg_id}, timeout=5)
    except:
        pass
...
def get_name(chat_id, user_id):
    try:
        r = requests.post(f"https://api.telegram.org/bot{TOKEN}/getChatMember", data={"chat_id": chat_id, "user_id": user_id}, timeout=10).json()
        if r.get("ok"):
            u = r["result"]["user"]
...
print("✅ ربات M2_GuardBot با سرعت ۱.۵ ثانیه روشن شد!")
    ...
    try:
        ...
        for update in r.get("result", []):
            ...
            if user_id == OWNER_ID or is_admin(chat_id, user_id) or is_group_owner(chat_id, user_id):
                ...
                # خاموشی/روشن (فقط OWNER یا مالک گروه)
                ...
                # ====== پاک کردن پیام (بدون ارسال پیام جدید) ======
                ...
                if text == "بن کن" and msg.get("reply_to_message"):
                    ...
                    requests.post(f"https://api.telegram.org/bot{TOKEN}/banChatMember", data={"chat_id": chat_id, "user_id": target}, timeout=10)
        ...
        time.sleep(1.5)  # افزایش به ۱.۵ ثانیه
