...
TOKEN = "8643184466:AAG9gyVjSYLVV9wYgTiTGSVawM2vZIIJh6U"  # توکن مستقیم
OWNER_ID = 7353819350
...
def can_send(chat_id, text):
    key = f"{chat_id}_{text[:30]}"
    now = time.time()
    if key in sent_cache and now - sent_cache[key] < 3:
        return False
    sent_cache[key] = now
    return True

def send_msg(chat_id, text, reply_to=None, delete_after=60):
    if not can_send(chat_id, text):
        return None
...
print("✅ ربات M2_GuardBot با رفع دوبار ارسال روشن شد!")
...
while True:
    try:
        ...
        for update in r.get("result", []):
            ...
            # ====== فیکس اصلی: آفست رو هر بار جلو می‌بریم تا آپدیت تکراری نگیریم ======
            last_update = uid
