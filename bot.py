...
# ====== لیست کلمات بد ======
bad_words = [
    "فحش", "کیر", "کص", "کس", "کون", "گوه", "گه", "خایه", "جنده", 
    "حرومزاده", "پدرسگ", "مادرجنده", "بی‌شرف", "ناموس", "لاشی",
    "هرزه", "فاحشه", "کونی", "کسخل", "کسکش", "گوزو", "پدرتو",
    "مادرتو", "خواهرتو", "گشاد", "سگ", "خوک", "الاغ", "خر", "ممه"
]
...
print("✅ ربات M2_GuardBot با دستور 'خفه' روشن شد!")
        ...
        for update in r.get("result", []):
            ...
            if is_user_admin:
                ...
                if text.startswith("خفه") and msg.get("reply_to_message"):
                    ...
                    try:
                        ...
                        if minutes > 0:
                            ...
                            send_message(chat_id, f"🔇 کاربر {minutes} دقیقه خفه شد!")
                        else:
                ...
                if text == "آزاد کن" and msg.get("reply_to_message"):
                    ...
                    send_message(chat_id, "🔊 خفه‌گی کاربر برداشته شد!")
                    continue
                ...
                if text == "بن کن" and msg.get("reply_to_message"):
                    ...
                    requests.post(url, data=data)
                    send_message(chat_id, "🚫 کاربر بن شد!")
                    continue
            ...
            if text:
                for word in bad_words:
                    if word in text:
                        ...
                        send_message(chat_id, f"🚫 پیام حاوی '{word}' پاک شد! ۱ دقیقه خفه شدی.")
                        break
            ...
            if "animation" in msg:
                ...
                if gif_id in bad_gifs:
                    ...
                    send_message(chat_id, "🚫 گیف بد پاک شد! ۳ دقیقه خفه شدی.")
            ...
            if "sticker" in msg:
                ...
                if sticker_id in bad_stickers:
                    delete_message(chat_id, message_id)
                    mute_user(chat_id, user_id, 3)
                    send_message(chat_id, "🚫 استیکر بد پاک شد! ۳ دقیقه خفه شدی.")
