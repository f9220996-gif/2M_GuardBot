...
def send_msg(chat_id, text, reply_to=None, delete_after=0.1):
...
def send_msg_keep(chat_id, text, reply_to=None):
    """ارسال پیام بدون پاک‌کاری (برای سکوت/بن که باید بمونه)"""
    key = f"{chat_id}_{text[:50]}"
    now = time.time()
    if key in sent_cache and now - sent_cache[key] < 0.1:
        return None
...
while True:
    try:
        ...
        for update in r.get("result", []):
            ...
            # ====== پاک کردن پیام تکراری ربات (۰.۱ ثانیه) ======
            ...
            # ====== پیام راهنما (۰.۱ ثانیه) ======
            if chat_type == "private":
                if text == "/start":
                    if user_id == OWNER_ID:
                        send_msg(chat_id, HELP_TEXT, reply_to=msg_id, delete_after=0.1)
                    continue
            else:
                if text == "/start":
                    continue
                ...
                if not is_authorized(chat_id, user_id):
                    ...
                    if user_id == OWNER_ID or is_group_owner(chat_id, user_id):
                        if text == "خاموشی":
                            group_locked = True
                            send_msg(chat_id, "🔒 گروه خاموش شد!", reply_to=msg_id, delete_after=0.1)
                            continue
                        if text == "روشن":
                            group_locked = False
                            send_msg(chat_id, "🔓 گروه روشن شد!", reply_to=msg_id, delete_after=0.1)
                            continue
                    ...
                    if text == "گیف بد" and msg.get("reply_to_message"):
                        ...
                        if gif:
                            ...
                            send_msg(chat_id, "✅ گیف اضافه شد!", reply_to=msg_id, delete_after=0.1)
                        continue
                    if text == "استیکر بد" and msg.get("reply_to_message"):
                        ...
                        if sticker:
                            ...
                            send_msg(chat_id, "✅ استیکر اضافه شد!", reply_to=msg_id, delete_after=0.1)
                        continue
                    
                    # ====== سکوت (با پیام ماندگار) ======
                    if text.startswith("سکوت") and msg.get("reply_to_message"):
                        ...
                        try:
                            ...
                            if mins > 0:
                                ...
                                send_msg_keep(chat_id, f"🔇 {get_name(chat_id, target)} {mins} دقیقه سکوت شد! (تا {format_time(muted_users[target])})", reply_to=msg_id)
                        except:
                            send_msg(chat_id, "❗ سکوت 5", reply_to=msg_id, delete_after=0.1)
                        continue
                    
                    # ====== آزاد کن (با پیام ماندگار) ======
                    if text == "آزاد کن" and msg.get("reply_to_message"):
                        ...
                        send_msg_keep(chat_id, f"🔊 {get_name(chat_id, target)} آزاد شد!", reply_to=msg_id)
                        continue
                    
                    # ====== بن کن (با پیام ماندگار) ======
                ...
                if chat_type != "private":
                    for new_member in msg["new_chat_members"]:
                        ...
                        if uid_new and uid_new != OWNER_ID:
                            ...
                            send_msg(chat_id, welcome_text, reply_to=msg_id, delete_after=0.1)
                continue
            
            # ====== پیوی فقط برای OWNER (۰.۱ ثانیه) ======
            if chat_type == "private":
                if user_id == OWNER_ID:
                    if text in owner_phrases:
                        send_msg(chat_id, owner_phrases[text], reply_to=msg_id, delete_after=0.1)
                    elif text == "تاریخ":
                        send_msg(chat_id, f"📅 {get_date()}", reply_to=msg_id, delete_after=0.1)
                continue
            
            # ====== تاریخ برای همه (۰.۱ ثانیه) ======
            if text == "تاریخ":
                send_msg(chat_id, f"📅 {get_date()}", reply_to=msg_id, delete_after=0.1)
                continue
            
            # ====== OWNER در گروه (۰.۱ ثانیه) ======
            if user_id == OWNER_ID:
                if text in owner_phrases:
                    send_msg(chat_id, owner_phrases[text], reply_to=msg_id, delete_after=0.1)
                    continue
            ...
            if text:
                for w in check_words:
                    if w in text:
                        if user_id != OWNER_ID:
                            send_msg(chat_id, "آقامون نیستن 😔", reply_to=msg_id, delete_after=0.1)
                        break
            ...
            if text and user_id != OWNER_ID and not is_admin(chat_id, user_id):
                for w in bad_words:
                    if w in text:
                        ...
                        if c == 1:
                            send_msg(chat_id, "⚠️ اخطار ۱", reply_to=msg_id, delete_after=0.1)
                        elif c == 2:
                            send_msg(chat_id, "⚠️⚠️ اخطار ۲", reply_to=msg_id, delete_after=0.1)
                        elif c == 3:
                            send_msg(chat_id, "⚠️⚠️⚠️ اخطار ۳", reply_to=msg_id, delete_after=0.1)
                        elif c >= 4:
                            mute(chat_id, user_id, 5)
                            send_msg_keep(chat_id, f"🚫 {get_name(chat_id, user_id)} ۵ دقیقه سکوت شد! (تا {format_time(muted_users[user_id])})", reply_to=msg_id)
            ...
            if "animation" in msg and user_id != OWNER_ID and not is_admin(chat_id, user_id):
                ...
                if gif_id in bad_gifs:
                    ...
                    if c >= 4:
                        mute(chat_id, user_id, 5)
                        send_msg_keep(chat_id, f"🚫 {get_name(chat_id, user_id)} ۵ دقیقه سکوت شد!", reply_to=msg_id)
                        user_warnings[user_id] = 0
