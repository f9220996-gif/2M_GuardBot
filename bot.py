...
def send_msg(chat_id, text, reply_to=None, delete_after=0.5):
    key = f"{chat_id}_{text[:50]}"
    now = time.time()
    if key in sent_cache and now - sent_cache[key] < 0.5:
        return None
    sent_cache[key] = now
...
print("✅ ربات M2_GuardBot با پاک‌کاری ۰.۵ ثانیه روشن شد!")
    ...
    try:
        ...
        for update in r.get("result", []):
            ...
            if chat_type == "private":
                if text == "/start":
                    if user_id == OWNER_ID:
                        send_msg(chat_id, HELP_TEXT, reply_to=msg_id, delete_after=0.5)
                    continue
            else:
                if text == "/start":
                    continue
                ...
                if not is_authorized(chat_id, user_id):
                    ...
                    if user_id == OWNER_ID or is_group_owner(chat_id, user_id):
                        if text == "خاموشی":
                            group_locked = True
                            send_msg(chat_id, "🔒 گروه خاموش شد!", reply_to=msg_id, delete_after=0.5)
                            continue
                        if text == "روشن":
                            group_locked = False
                            send_msg(chat_id, "🔓 گروه روشن شد!", reply_to=msg_id, delete_after=0.5)
                            continue
                    ...
                    if text == "گیف بد" and msg.get("reply_to_message"):
                        ...
                        if gif:
                            ...
                            send_msg(chat_id, "✅ گیف اضافه شد!", reply_to=msg_id, delete_after=0.5)
                        continue
                    if text == "استیکر بد" and msg.get("reply_to_message"):
                        ...
                        if sticker:
                            ...
                            send_msg(chat_id, "✅ استیکر اضافه شد!", reply_to=msg_id, delete_after=0.5)
                        continue
                    ...
                    if text.startswith("سکوت") and msg.get("reply_to_message"):
                        ...
                        if not can:
                            send_msg(chat_id, error, reply_to=msg_id, delete_after=0.5)
                            continue
                        try:
                            mins = int(text.replace("سکوت", "").strip())
                            if mins > 0:
                                mute(chat_id, target, mins)
                                send_msg(chat_id, f"🔇 {get_name(chat_id, target)} {mins} دقیقه سکوت شد! (تا {format_time(muted_users[target])})", reply_to=msg_id, delete_after=0.5)
                        except:
                            send_msg(chat_id, "❗ سکوت 5", reply_to=msg_id, delete_after=0.5)
                        continue
                    ...
                    if text == "آزاد کن" and msg.get("reply_to_message"):
                        ...
                        send_msg(chat_id, f"🔊 {get_name(chat_id, target)} آزاد شد!", reply_to=msg_id, delete_after=0.5)
                        continue
                    ...
                    if text == "بن کن" and msg.get("reply_to_message"):
                        ...
                        send_msg(chat_id, f"🚫 {get_name(chat_id, target)} بن شد!", reply_to=msg_id, delete_after=0.5)
                        continue
                ...
                if chat_type != "private":
                    for new_member in msg["new_chat_members"]:
                        ...
                        if uid_new and uid_new != OWNER_ID:
                            ...
                            send_msg(chat_id, welcome_text, reply_to=msg_id, delete_after=0.5)
                continue
            ...
            if chat_type == "private":
                if user_id == OWNER_ID:
                    if text in owner_phrases:
                        send_msg(chat_id, owner_phrases[text], reply_to=msg_id, delete_after=0.5)
                    elif text == "تاریخ":
                        send_msg(chat_id, f"📅 {get_date()}", reply_to=msg_id, delete_after=0.5)
                continue
            ...
            if text:
                for w in check_words:
                    if w in text:
                        if user_id != OWNER_ID:
                            send_msg(chat_id, "آقامون نیستن 😔", reply_to=msg_id, delete_after=0.5)
                        break
            ...
            if text and user_id != OWNER_ID and not is_admin(chat_id, user_id):
                for w in bad_words:
                    if w in text:
                        ...
                        if c == 1:
                            send_msg(chat_id, "⚠️ اخطار ۱", reply_to=msg_id, delete_after=0.5)
                        elif c == 2:
                            send_msg(chat_id, "⚠️⚠️ اخطار ۲", reply_to=msg_id, delete_after=0.5)
                        elif c == 3:
                            send_msg(chat_id, "⚠️⚠️⚠️ اخطار ۳", reply_to=msg_id, delete_after=0.5)
                        elif c >= 4:
                            mute(chat_id, user_id, 5)
                            send_msg(chat_id, f"🚫 {get_name(chat_id, user_id)} ۵ دقیقه سکوت شد! (تا {format_time(muted_users[user_id])})", reply_to=msg_id, delete_after=0.5)
            ...
            if "animation" in msg and user_id != OWNER_ID and not is_admin(chat_id, user_id):
                ...
                if gif_id in bad_gifs:
                    ...
                    if c >= 4:
                        mute(chat_id, user_id, 5)
                        send_msg(chat_id, f"🚫 {get_name(chat_id, user_id)} ۵ دقیقه سکوت شد!", reply_to=msg_id, delete_after=0.5)
                        user_warnings[user_id] = 0
